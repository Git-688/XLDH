// admin.js - 星聚导航后台管理（完整版，日志从后端API获取）
(function() {
    ...
    // 日志存储配置（不再使用 localStorage，改为从后端获取）
    let token = '';
    ...
    async function showLogs() {
        const logs = await loadLogsFromServer();
        const logListHtml = logs.length
            ? logs.map(l => `<div class="log-item"><span>${escapeHtml(new Date(l.timestamp).toLocaleString())}</span> ${escapeHtml(l.action)} - ${escapeHtml(l.details)} (${escapeHtml(l.ip)})</div>`).join('')
