(function() {
    ...
    const { escapeHtml, showToast, checkUrl, autoResizeTextarea, getDeviceId, normalizeUrl, isValidEmail } = Utils;
    ...
    function isUrlExists(url, excludeSiteId = null) {
        ...
        return sites.some(site => {
            ...
            return normalizeUrl(site.url) === normalizedNew;
        });
    }
    ...
    function updateLockMessage(locked) {
        ...
        if (!el) return;
        el.textContent = locked ? '登录失败过多，请10分钟后重试' : '';
    ...
    async function loadAdminSites(resetPage = true) {
        ...
        try {
            ...
            if (currentSub) url += `&subcategory_id=${currentSub}`;
    ...
    function renderSitesWithCheckboxes(sitesData) {
        ...
        listEl.innerHTML = sitesData.map(site => {
            ...
            return `
                <div class="site-card-admin" style="background:#fff;border-radius:8px;padding:10px;border:1px solid #eef2f6;display:flex;flex-direction:column;justify-content:space-between;min-height:80px;transition:border-color .2s,box-shadow .2s;">
                    ...
                    <div style="flex:1;display:flex;align-items:center;justify-content:center;padding:4px 0 2px;">
        ...
        listEl.querySelectorAll('[data-action="editSite"]').forEach(btn => {
            btn.addEventListener('click', e => { e.stopPropagation(); handleEditSite(parseInt(this.dataset.id)); });
        });
    }
    ...
    async function batchDeleteSites() {
        ...
        if (!confirm(`确定要删除选中的 ${selectedSiteIds.size} 个站点吗？`)) return;
    ...
    async function batchMoveSites() {
        ...
        openModal('批量移动站点',
            `<div style="margin-bottom:12px;"><label style="display:block;font-size:12px;margin-bottom:6px;color:#475569;">选择目标子分类：</label><div id="batchMoveTargetWrapper"></div></div>
            ...
            function() { if (batchMoveCustomSelect) { batchMoveCustomSelect.destroy(); batchMoveCustomSelect = null; } }
        );
    }
    ...
    async function loadAnnouncement() {
        try {
            ...
            if (data && data.length > 0) {
                ...
                autoResizeTextarea(document.getElementById('annContent'));
            } else {
    ...
    async function saveAnnouncement() {
        ...
        try {
            ...
            if (window.announcementModule?.loadAnnouncement) window.announcementModule.loadAnnouncement();
    ...
    async function getNextSortValue(type, parentId = null) {
        try {
            let maxOrder = 0;
            if (type === 'category') maxOrder = categories.reduce((max, c) => Math.max(max, c.display_order || 0), 0);
            else if (type === 'subcategory' && parentId) maxOrder = subcategories.filter(s => s.category_id === parentId).reduce((max, s) => Math.max(max, s.display_order || 0), 0);
            else if (type === 'site' && parentId) maxOrder = sites.filter(s => s.subcategory_id === parentId).reduce((max, s) => Math.max(max, s.display_order || 0), 0);
            return maxOrder + 1;
        } catch { return 0; }
    }
    ...
    function openModal(title, formHtml, submitCb, showDelete = false, deleteCb = null, onShow = null, onClose = null) {
        ...
        document.getElementById('modalCancelBtn').addEventListener('click', function() { closeModal(); if (onClose) onClose(); });
    ...
    function fetchSiteInfoHandler() { fetchSiteInfo('mUrl', 'mTitle', 'mIcon', 'mDesc'); }
    ...
    async function handleAddSite() {
        ...
        openModal('新增链接',
            `<div class="form-row" style="margin-bottom:12px;"><label style="font-size:11px;">标题</label><input id="mTitle" class="form-input"></div>
             ...
             <div class="form-row"><label style="font-size:11px;">排序值</label><input type="number" id="mSort" class="form-input" value="${nextOrder}" step="1"></div>`,
            async () => {
                const title = document.getElementById('mTitle').value.trim();
                const url = document.getElementById('mUrl').value.trim();
                if (!title || !url) { showToast('标题和网址必填', 'error'); return; }
                ...
                await apiFetch('/admin/sites', { method:'POST', body: JSON.stringify({
                    subcategory_id: currentSub, title, url,
    ...
    async function loadRanking() {
        ...
        try {
            ...
            list.innerHTML = data.map(item => `
                <div class="link-item"><div class="link-info"><strong>${escapeHtml(item.title)}</strong></div><span class="badge badge-blue">${item.count}次</span></div>
            `).join('');
    ...
    async function replaceLinkHandler(e) {
        ...
        setTimeout(() => { autoResizeTextarea(document.getElementById('mDesc')); }, 50);
    }
    ...
    function openImportModal() { document.getElementById('importModal').classList.add('show'); document.getElementById('importFile').value = ''; }
    ...
    async function fetchSubmissions(forceRefresh = false) {
        const now = Date.now();
        if (!forceRefresh && cachedSubmissions && (now - submissionsCacheTime) < SUBMISSIONS_CACHE_TTL) return cachedSubmissions;
    ...
    function cleanupCustomSelects() {
        if (customSelects.cat && typeof customSelects.cat.destroy === 'function') customSelects.cat.destroy();
        if (customSelects.sub && typeof customSelects.sub.destroy === 'function') customSelects.sub.destroy();
        customSelects = {};
        if (batchMoveCustomSelect) { batchMoveCustomSelect.destroy(); batchMoveCustomSelect = null; }
    }
    ...
    async function openSubmissionDetail(id) {
        ...
        try {
            ...
            if (!item) { showToast('未找到该投稿（可能已被删除）', 'error'); return; }
            ...
            contentDiv.innerHTML = `
                ...
                <div class="action-section" style="display:flex;gap:12px;flex-wrap:wrap;">
                    <div style="flex:1;background:#f8fafc;padding:12px;border-radius:12px;">
                        ...
                        <div style="margin:10px 0 0 0;">
                            <label style="display:inline-flex;align-items:center;gap:6px;font-size:11px;">
                                <input type="checkbox" id="sendEmailCheckbox" checked> 📧 发送邮件通知
                            </label>
                        </div>
                    ...
                    <div style="flex:1;background:#f8fafc;padding:12px;border-radius:12px;">
                        ...
                        <div style="margin:10px 0 0 0;">
                            <label style="display:inline-flex;align-items:center;gap:6px;font-size:11px;">
                                <input type="checkbox" id="sendEmailCheckboxReject" checked> 📧 发送邮件通知
                            </label>
                        </div>
            ...
            document.getElementById('approveCatSelectWrapper').appendChild(catSelect);

            const subSelect = document.createElement('select');
            subSelect.id = 'approveSubSelect';
            subSelect.innerHTML = '<option value="">先选择一级分类</option>';
            document.getElementById('approveSubSelectWrapper').appendChild(subSelect);

            customSelects.cat = new CustomSelect(catSelect, async (value) => {
                subSelect.innerHTML = '<option value="">加载中...</option>';
                if (customSelects.sub) { customSelects.sub.destroy(); customSelects.sub = null; }
                if (value) {
                    try {
            ...
            document.getElementById('doApproveBtn').onclick = async () => {
                ...
                try {
                    await apiFetch(`/admin/submissions/${id}/approve`, {
                        method: 'POST',
                        body: JSON.stringify({
                            subcategory_id: parseInt(subId), display_order: parseInt(displayOrder),
                            title: editedTitle, url: editedUrl, icon: editedIcon,
                            description: editedDesc, contact: editedContact, sendEmail
                        })
                    });
    ...
    async function refreshIcons() {
        ...
        try {
            ...
            const data = await response.json();
            if (response.ok) showToast(data.message || '图标缓存已刷新', 'success');
            else showToast(data.error || '刷新失败', 'error');
        } catch (err) { showToast('刷新失败: ' + err.message, 'error'); }
        finally { btn.disabled = false; btn.textContent = originalText; }
    }

    function refreshNavigationStats() {
        if (window.optimizedNavigation?.calculateTotalValidSites) {
            window.optimizedNavigation.calculateTotalValidSites().catch(() => {});
        }
    }
    ...
    class CustomSelect {
        ...
        populateOptions() {
            ...
            for (let i = 0; i < this.select.options.length; i++) {
                ...
                div.dataset.index = i;
                div.addEventListener('click', (e) => { e.stopPropagation(); this.selectOption(i); this.close(); });
        ...
        setValue(value) {
            for (let i = 0; i < this.select.options.length; i++) {
                if (this.select.options[i].value == value) { this.selectOption(i); break; }
            }
        }
    ...
    function setupEventDelegation() {
        document.getElementById('catBar').addEventListener('click', e => {
            ...
            if (btn && btn.dataset.action === 'modifyCat') { handleModifyCategory(parseInt(btn.dataset.id), btn.dataset.name); return; }
            const item = e.target.closest('.cat-item');
            if (item) selectCat(parseInt(item.dataset.cid));
        });
        document.getElementById('subList').addEventListener('click', e => {
            const btn = e.target.closest('[data-action]');
            if (btn && btn.dataset.action === 'modifySub') { handleModifySub(parseInt(btn.dataset.id), btn.dataset.name); return; }
        ...
        document.getElementById('refreshNavBtn').addEventListener('click', async () => {
            try { await apiFetch('/admin/refresh-navigation', { method: 'POST' }); showToast('导航缓存已刷新', 'success'); refreshNavigationStats(); }
        ...
        document.getElementById('annCancelBtn').addEventListener('click', () => { currentAnnouncement ? loadAnnouncement() : clearAnnouncementForm(); });
