(function() {
    ...
    function renderCatBar() {
        if (!categories.length) {
            document.getElementById('catBar').innerHTML = '<div class="empty">请先添加一级分类</div>';
    ...
    function renderSiteList() {
        ...
        document.getElementById('siteList').innerHTML = list.map(s => `
            <div class="link-item">
                <div class="link-info"><div>${escapeHtml(s.title)}</div><div class="link-url">${escapeHtml(s.url)}</div></div>
    ...
    function setupEventDelegation() {
        ...
        document.getElementById('catBar').addEventListener('click', e => {
            ...
            if (e.target.closest('[data-action]')) {
                ...
                if (action === 'renameCat') renameCategory(id, e.target.dataset.name);
                else if (action === 'delCat') deleteCategory(id);
                return;
            }
            selectCat(parseInt(item.dataset.cid));
        });
        ...
        document.getElementById('subList').addEventListener('click', e => {
            ...
            if (e.target.closest('[data-action]')) {
                const action = e.target.dataset.action;
                const id = parseInt(e.target.dataset.id);
                if (action === 'renameSub') renameSubcategory(id, e.target.dataset.name);
                else if (action === 'delSub') deleteSubcategory(id);
                return;
            }
            selectSub(parseInt(item.dataset.sid));
        });
    ...
    async function replaceLink(reportId, siteId, oldUrl, oldTitle) {
        ...
        `, async () => {
            ...
            if (!newTitle || !newUrl) { showToast('标题和网址必填', 'error'); return; }
    ...
    // 获取网站 favicon 的辅助函数（暴露给 window，因为内联 oninput 调用）
    ...
    setInterval(() => {
        const exp = sessionStorage.getItem('admin_expires');
        if (exp && Date.now() > parseInt(exp, 10) - 60000) {
    ...
    // 暴露关闭模态框函数（供 HTML onclick 引用）
    window.closeModalMain = closeModal;
    window.closeLogModalMain = closeLogModal;
})();